...
def get_model_info(year):
    ...
    for model in models:
        ...
        print 'Brand: ' + model.brand.name,
        print 'Headquarters: ' + model.brand.headquarters
...
def get_brands_summary():
    ...
    brands = Model.query.order_by('brand_id').all()
    for i, b in enumerate(brands):
        if b.brand_id != brands[i-1].brand_id:
            print b.brand.name
            print '\t' + b.name,
            print b.year
        else:
...
def search_brands_by_name(mystr):
    ...
    m_brand = Brand.query.filter(Brand.name.like(%mystr%)).all()
    return m_brand

    # if I wanted to print something from the object, I would loop over and print
    # with format for boo in fah: print boo.classmethod
